...
Claude Code integrates with Neovim via claudecode.nvim.

### Launch
| Action | Keys | Command |
|--------|------|---------|
| Toggle | `<leader>ac` | `claude` |
| Focus | `<leader>af` | Focus terminal |
| Resume | `<leader>ar` | `claude --resume` |

### Selection & Diffs
| Action | Keys | Mode |
|--------|------|------|
| Send to Claude Code | `<leader>as` | Visual |
...
Configure the terminal position in `claudecode.lua`:
...
### Configuration Note
The plugin uses a **full path** instead of an alias, because Neovim's terminal
does not load `.zshrc` and `~/.local/bin` is not on its `PATH`:
...
If you get "exit code 127" (command not found), check that `~/.local/bin/claude`
exists and is executable.

### Content Features (Claude Code ↔ Neovim)

claudecode.nvim creates a **two-way bridge** between Claude Code and Neovim via WebSocket/MCP protocol.

#### Send Selection to Claude Code
Select code in visual mode and send it to Claude Code for discussion:
```
1. Visual select code (v, V, or Ctrl+v)
2. Press <leader>as
3. Selection is sent to Claude Code with file context
4. Claude Code can analyze, explain, or suggest changes
```

#### Claude Code Opens Files in Neovim
When Claude Code references a file, it can open it directly in your editor:
- Claude Code says "let me show you line 42 of config.lua"
- File opens in Neovim at that line
- No manual navigation needed

#### Claude Code Proposes Diffs
When Claude Code suggests code changes, they appear as a diff view:
```
1. Claude Code proposes changes → diff view opens
...
#### Selection Tracking
Claude Code always knows what you're looking at:
- Current file path
- Cursor position
...
#### Diagnostics Access
Claude Code can read LSP diagnostics:
...
| Setting | Issue | Fix |
| ------- | ----- | --- |
| `escape-time` (missing) | Old default 500ms delay after Escape created input lag | Set `set -s escape-time 10` (modern upstream default; 0 risks misparsed escapes) |
...
3. **Pane won't focus?** → Check for `-r` flags on tmux bindings + a sane `escape-time` (10)
