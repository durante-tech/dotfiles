...
for a in "$@"; do
  case "$a" in
    ...
    *) echo "Unknown layout option: $a" >&2; exit 2 ;;
  esac
done

# Internal helper: public callers enter through display-restore.sh and the
# controller lock. An accidental direct invocation must not write displays.
if [[ "$DRY_RUN" != true && "${DOTFILES_DISPLAY_LAYOUT_LOCKED:-}" != 1 ]]; then
  echo "Use display-restore.sh so layout changes take the controller lock" >&2
  exit 75
fi
...
[[ "$DRY_RUN" == true ]] && { printf 'would apply:\n'; printf '  %s\n' "${args[@]}"; exit 0; }
...
drifted() {
  ...
  for line in "${args[@]}"; do
    ...
    cur="$(awk -v u="$id" '
      ...
      f&&/Scaling:/{s=$2}
      f&&/Origin:/{o=$2}
      f&&/Rotation:/{print r"|"s"|"o"|"$2; exit}' <<< "$DP_LIST")"
...
if drifted "$ACTION"; then
  ...
  if ! "$DP" "${args[@]}" >>"$LOG" 2>&1; then
    log "ERROR displayplacer failed; prior profile retained"
    echo "Layout apply failed; see $LOG" >&2
    exit 1
  fi
  DP_LIST="$("$DP" list 2>/dev/null)" || exit 1
  if drifted; then
    log "ERROR layout readback differs; prior profile retained"
    echo "Layout did not reach the requested values" >&2
    exit 1
  fi
  log "restored and verified"
else
  log "layout already canonical — no-op"
fi

# Only publish a confirmed profile. The parent controller holds the shared lock.
PROFILE_DIR="${DOTFILES_DISPLAY_STATE_DIR:-$HOME/.cache}"
mkdir -p "$PROFILE_DIR" || exit 1
PROFILE_TMP="$(mktemp "$PROFILE_DIR/.bd-profile.XXXXXX")" || exit 1
trap 'rm -f "$PROFILE_TMP"' EXIT
printf '%s\n' "$PROFILE" > "$PROFILE_TMP" && mv "$PROFILE_TMP" "$PROFILE_DIR/bd-profile" || exit 1
