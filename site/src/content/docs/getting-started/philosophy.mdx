---
title: "Philosophy & Design Decisions"
description: "Why this system exists and the principles behind every choice."
---
...
See [GNU Stow Guide](/getting-started/gnu-stow/) for details. The key insight: each directory in this repo mirrors `$HOME`. Stow creates symlinks, so editing `~/dotfiles/nvim/.config/nvim/init.lua` is the same as editing `~/.config/nvim/init.lua`.
...
<CompleteLessonButton client:idle lessonId="0.1" slug="getting-started/philosophy" nextLessonSlug="getting-started/installation" nextLessonTitle="Installation" />
